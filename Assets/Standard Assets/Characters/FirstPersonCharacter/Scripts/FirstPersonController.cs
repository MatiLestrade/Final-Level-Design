...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (dashBool && Input.GetKeyDown(KeyCode.F) && _count == 2) {
                if (_timer <= 0)
                {
                    /*      _rig.AddForce(new Vector3(0, 0, 20), ForceMode.Acceleration);*/
                    transform.position += transform.forward * m_WalkSpeed;
                    transform.forward += Vector3.forward * Time.deltaTime;
                    Debug.Log("anda");
                    _count = 0; }
                }
            if(!dashBool) _timer += Time.deltaTime;
            if (_timer >= 2) { dashBool = true; _timer = 0; }
